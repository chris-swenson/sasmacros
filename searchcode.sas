%macro SearchCode(code,term,out=Code_results,subdir=N,subex=,skip=N,pm=Y)
    / des='Search code for term';
     ...
     ********************************************************************************
        ...
        Arguments:  code    - specific code or directory of code to search
                    ...
                    skip=   - Y = Yes, skip any files that could not be imported
                              N = No, do not skip any files (default)
                    pm=     - Y = Yes, display the pop-up message (default)
        ...
        2011-11-08  CAS     Added process to skip files that could not be imported
                            instead of ending the macro. A new argument, SKIP=, sets
                            which style to use, defaulted to N for the original style.
        2012-02-29  CAS     Updated SCAN functions to QSCAN to avoid issues with
    ...
    %let SKIP=%upcase(&SKIP);
    %if %index(*Y*N*,*&SKIP*)=0 %then %do;
        %put %str(E)RROR: %str(I)nvalid argument specified for SKIP.;
    ...
    %else %if %index(&code,%str(.)) ge 1 %then %do;
        ...
        %if %sysfunc(fileexist( %substr(&code, 1, %eval( %length(&code) - %length( %qscan(&code, -1, %str(\)) ) )) ))
    ...
    %do codenumber=1 %to &codecount;
        ...
        /* REVISION 2011-11-08 CAS: Added process to skip instead of end the macro */
        ...
        /* REVISION 2012-02-29 CAS: Updated to use QSCAN */
        %let codename=%qscan(%qscan(%superq(code), -2, %str(.)), -1, %str(\));
    ...
    %end;
        ...
        %if &outobs ge 1 %then %do;
            %let results=Results: The term %superq(term) was found in %superq(codename). See Code_search for more details.;
        %end;
        %else %do;
            %let results=No results: The term %superq(term) was not found in %superq(codename).;
        %end;

        /* REVISION 2011-11-08 CAS: Added skip location */
        %skip:
        ...
        %if &multi=1 %then %do;
            ...
            %if &codenumber=&codecount %then %do;
                ...
                %if &outobs ge 1 %then %do;
                    %let results=Results: The term %superq(term) was found in the code. See &OUT for more details.;
                %end;
                %else %do;
