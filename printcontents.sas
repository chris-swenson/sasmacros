%macro PrintContents(table,short=N,style=MessageBox,test=N) 
    / des="Print the contents of the specified table(s)";
                ...
                %if &cbok=1 and &lastok=1 %then %do;
                    ...
                    %if &STYLE=M %then %do;
                        ...
                        %let response=%MessageBox(
                            ...
                            , title=PrintContents Question, buttons=YNC, default=1, icon=Q)
                        ;
                    ...
                    %else %if &STYLE=W %then %do;
                        ...
                        %window asktbl
                        #2 @5 "&wrong" color=red
                        #3 @5 "PrintContents: Select a Table: " asktbl 1 color=blue attr=rev_video required=yes " (C=Copied, L=Last, X=Cancel)"
